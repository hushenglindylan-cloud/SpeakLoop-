...
export default function InterviewPage() {
  ...
  // --- TTS preloading for next question ------------------------------------
  // While the student answers the current question, pre-generate TTS for the
  // next main question so it's ready to play immediately when we advance.
  // Keyed by question index to avoid stale audio from a previous session.
  const ttsCacheRef = useRef<Map<number, string>>(new Map());
  const ttsLoadingRef = useRef<Set<number>>(new Set());
  ...
  // Clear TTS cache when questions change (new session) or on unmount
  useEffect(() => {
    if (questions.length > 0) {
      ttsCacheRef.current.clear();
      ttsLoadingRef.current.clear();
    }
  }, [questions]);
  ...
  // Pre-generate TTS for a future question (non-blocking).
  // Cached by question index so the audio is ready when we advance.
  const preloadTts = useCallback(
    (questionIndex: number, text: string) => {
      if (!examinerGender) return;
      if (ttsCacheRef.current.has(questionIndex)) return;
      if (ttsLoadingRef.current.has(questionIndex)) return;

      ttsLoadingRef.current.add(questionIndex);
      ...
      fetch('/api/tts', {
        ...
        .then((data) => {
          if (data?.audioUrl) {
            ttsCacheRef.current.set(questionIndex, data.audioUrl);
          }
        })
        .catch((err) => {
          console.error(`TTS preload failed for Q${questionIndex}:`, err);
        })
        .finally(() => {
          ttsLoadingRef.current.delete(questionIndex);
        });
    },
    [examinerGender]
  );
  ...
  const speakQuestion = useCallback(
    async (text: string, questionIndex: number) => {
      ...
      // No cached audio — generate on demand
  ...
  useEffect(() => {
    ...
    preloadTts(nextQ, nextQuestion.question);
