...
// Core evaluation — returned by Fast Evaluation API
interface CoreEvaluationData {
  scores: {
...
// ---------------------------------------------------------------------------
// Loading skeleton for core evaluation
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Loading skeleton for detailed evaluation sections
// ---------------------------------------------------------------------------
...
export default function EvaluationPage() {
  ...
  // Two-phase state
  const [coreEvaluation, setCoreEvaluation] = useState<CoreEvaluationData | null>(null);
  const [detailedEvaluation, setDetailedEvaluation] = useState<DetailedEvaluationData | null>(null);
  ...
  // Phase 1: Fast Evaluation (core scores)
  const runCoreEvaluation = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/evaluate-interview', {
        ...
        body: JSON.stringify({
          ...
          examiner: getExaminerContext(),
        }),
      });

      const data = await response.json();
      if (coreFetchSeqRef.current !== seq) return;

      if (!data?.scores || typeof data.scores.fluencyCoherence !== 'number') {
        setCoreError(data?.error || 'Evaluation failed. Please try again.');
      } else {
        setCoreEvaluation(data);
        // Save core evaluation data for Practice and Final Evaluation pages
        try {
          // For Practice page (weakness + improvementFocus + criteriaScores)
          ...
          // For Final Evaluation page (full core evaluation)
          localStorage.setItem('speakloop_evaluation_core', JSON.stringify({
            scores: data.scores,
            overallBand: data.overallBand,
            mainWeakness: data.mainWeakness,
            improvementFocus: data.improvementFocus,
          }));
        } catch {
      ...
      console.error('Failed to fetch core evaluation:', err);
      if (coreFetchSeqRef.current !== seq) return;
      setCoreError('Failed to load evaluation. Please try again.');
    } finally {
      if (coreFetchSeqRef.current === seq) setCoreLoading(false);
    }
  }, [getExaminerContext]);

  // Phase 2: Detailed Evaluation (criteria analysis + improved answers)
  const runDetailedEvaluation = useCallback(async (coreData: CoreEvaluationData) => {
    const currentTranscripts = getAllTranscripts();
    ...
    try {
      const response = await fetch('/api/evaluate-interview-details', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          transcripts: currentTranscripts,
          examiner: getExaminerContext(),
          coreEvaluation: {
            scores: coreData.scores,
            overallBand: coreData.overallBand,
            mainWeakness: coreData.mainWeakness,
            improvementFocus: coreData.improvementFocus,
          },
        }),
      });

      const data = await response.json();
      if (detailsFetchSeqRef.current !== seq) return;

      if (!data?.criteriaAnalysis || typeof data.criteriaAnalysis !== 'object') {
        setDetailsError('Unable to load detailed analysis.');
      } else {
        setDetailedEvaluation(data);
      }
    } catch (err) {
      console.error('Failed to fetch detailed evaluation:', err);
      if (detailsFetchSeqRef.current !== seq) return;
      setDetailsError('Unable to load detailed analysis.');
    } finally {
      if (detailsFetchSeqRef.current === seq) setDetailsLoading(false);
    }
  }, [getExaminerContext]);

  // Start core evaluation on mount
  useEffect(() => {
    if (hasRequestedCoreRef.current) return;
    hasRequestedCoreRef.current = true;
    runCoreEvaluation();
  }, [runCoreEvaluation]);

  // Start detailed evaluation when core is available
  useEffect(() => {
    if (!coreEvaluation || hasRequestedDetailsRef.current) return;
    hasRequestedDetailsRef.current = true;
    runDetailedEvaluation(coreEvaluation);
  }, [coreEvaluation, runDetailedEvaluation]);

  // Retry handler for core evaluation
  const retryCoreEvaluation = useCallback(() => {
    hasRequestedCoreRef.current = false;
    hasRequestedDetailsRef.current = false;
    setDetailedEvaluation(null);
    setDetailsError(null);
    runCoreEvaluation();
  }, [runCoreEvaluation]);

  // Retry handler for detailed evaluation
  const retryDetailedEvaluation = useCallback(() => {
    if (!coreEvaluation) return;
    hasRequestedDetailsRef.current = false;
    setDetailsError(null);
    runDetailedEvaluation(coreEvaluation);
  }, [coreEvaluation, runDetailedEvaluation]);

  // Core loading state — full page loader
  if (coreLoading) {
    return (
  ...
  // Core error state — show error with retry
  if (coreError || !coreEvaluation) {
    return (
      <div className="min-h-screen bg-white">
        <header className="border-b border-slate-100 bg-white/80 backdrop-blur-sm">
          <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
        ...
        <main className="mx-auto max-w-5xl px-6 py-12 text-center">
          ...
          <p className="text-slate-500 mb-6">
            {coreError || 'No recordings available. Please complete the interview first.'}
          </p>
          {coreError && transcripts.length > 0 ? (
            <div className="flex flex-col items-center gap-3">
              <button
                onClick={retryCoreEvaluation}
  ...
  const { scores, overallBand } = coreEvaluation;
  const criteriaAnalysis = detailedEvaluation?.criteriaAnalysis;
  const improvedAnswers = detailedEvaluation?.improvedAnswers;
  ...
  // Build evidence list from criteriaAnalysis (if available)
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="space-y-6 lg:col-span-2">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-8">
              ...
              {/* Detailed evaluation loading/error state */}
              {detailsLoading && (
                <div className="mb-6 rounded-xl bg-slate-50 border border-slate-100 p-4">
                  <div className="flex items-center gap-2 mb-3">
                    <div className="h-4 w-4 animate-spin rounded-full border-2 border-slate-300 border-t-[#DA291C]" />
                    <p className="text-sm text-slate-500">Preparing detailed analysis...</p>
                  </div>
                  <DetailedLoadingSkeleton />
                </div>
              )}

              {detailsError && !detailsLoading && (
                <div className="mb-6 rounded-xl bg-amber-50 border border-amber-100 p-4">
                  <p className="text-sm text-amber-700 mb-2">{detailsError}</p>
                  <button
                    onClick={retryDetailedEvaluation}
                  ...
                  transcripts.map((t, idx) => {
                    ...
                    return (
                      <div key={idx} className="space-y-3">
                        ...
                        {/* Improved Version (from AI) — only show when available */}
                        {improved && !detailsLoading && (
                          <div>
